...
class SimulatedAnnealingAgent(Agent):
    ...
    def solve_problem(self, schedule):
        ...
        while temp > 0:
            ...
            # Determine If:
            # the agent should try to add a candidate to the solution
            # OR
            # remove a candidate from the solution
            add_candidate = (
                random.randint(0, 1) == 1 & (len(self.candidates) != 0)
            ) | len(self.solution) <= 1
            if add_candidate:  # Try to add a random candidate to the solution
                candidate = self.candidates.pop(
                    random.randint(0, (len(self.candidates) - 1))
                )
            else:  # Try to remove candidate from the solution
                candidate = self.solution.pop(
                    random.randint(0, (len(self.solution) - 1))
                )
            ...
            elif exp(-rewards_delta / temp) < random.randrange(0, 1) & (
                potential_future_rewards > 0
            ):
                self.candidates.append(candidate)  # Accept the removal of the candidate
            elif not add_candidate:
                self.solution.append(
                    candidate
                )  # Don't remove the candidate from the solution
            else:
                self.candidates.append(
                    candidate
                )  # Don't add the candidate to the solution
